...
import {T_DecreesFilters} from "modules/types.ts";
import DecreesTable from "components/DecreesTable/DecreesTable.tsx";
...
const DecreesPage = () => {
    ...
    const filters = useAppSelector<T_DecreesFilters>((state) => state.decrees.filters)
    ...
    useEffect(() => {
        const intervalId = setInterval(handleFetchDecrees, 2000)
        return () => clearInterval(intervalId)
    }, [filters]);

    const handleFetchDecrees = () => dispatch(fetchDecrees())

    const applyFilters = async (e) => {
        e.preventDefault()

        const filters:T_DecreesFilters = {
            status: status,
    ...
    return (
        <Container>
            ...
            {decrees.length ? <DecreesTable decrees={decrees} /> : <h3 className="text-center mt-5">Приказы не найдены</h3>}
        </Container>
    )
};
