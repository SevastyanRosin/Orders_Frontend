...
import {E_DecreeStatus, T_Decree, T_DecreesFilters, T_Unit} from "modules/types.ts";
...
export const fetchDecrees = createAsyncThunk<T_Decree[], object, AsyncThunkConfig>(
    ...
    async function(_, thunkAPI) {
        ...
        return response.data.filter(decree => decree.owner.includes(state.decrees.filters.owner))
    }
)
...
export const acceptDecree = createAsyncThunk<void, string, AsyncThunkConfig>(
    "decrees/accept_decree",
    async function(decree_id,{dispatch}) {
        await api.decrees.decreesUpdateStatusAdminUpdate(decree_id, {status: E_DecreeStatus.Completed})
        await dispatch(fetchDecrees)
    }
)

export const rejectDecree = createAsyncThunk<void, string, AsyncThunkConfig>(
    "decrees/accept_decree",
    async function(decree_id,{dispatch}) {
        await api.decrees.decreesUpdateStatusAdminUpdate(decree_id, {status: E_DecreeStatus.Rejected})
        await dispatch(fetchDecrees)
    }
)
